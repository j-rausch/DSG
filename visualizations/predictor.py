...
class VisualizationDemo(object):
    def __init__(self, cfg, instance_mode=ColorMode.IMAGE, parallel=False):
        ...
        self.num_rel_categories_without_bg = cfg.MODEL.ROI_SCENEGRAPH_HEAD.NUM_CLASSES
        self.filter_background_relations = cfg.MODEL.ROI_SCENEGRAPH_HEAD.APPLY_ARGMAX_OVER_ALL_LOGITS_AND_FILTER_FG
    ...
    def run_on_image(self, image):
        ...
        image = image[:, :, ::-1]
        visualizer = SGVisualizer(image, self.metadata, instance_mode=self.instance_mode)
...
#                instances = predictions["instances"].to(self.cpu_device)
#                vis_output = visualizer.draw_instance_predictions(predictions=instances)
            if 'rel_pair_idxs' in predictions:
                instances = predictions["instances"].to(self.cpu_device)
                ...
                vis_output_rels = visualizer.generate_relation_graph(instances=instances, rel_pair_idx=pred_rel_pair, pred_rel_scores=pred_rel_scores, pred_classes=pred_classes)

        return predictions, vis_output, vis_output_rels
