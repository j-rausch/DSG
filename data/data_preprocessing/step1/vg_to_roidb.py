...
def extract_object_token(data, num_tokens, obj_list=[], verbose=True, use_zero_freq_tokens=False):
    """ Builds a set that contains the object names. Filters infrequent tokens. """
    token_counter = Counter()
    #initialize
    if use_zero_freq_tokens:
        token_counter.update({x:0 for x in obj_list})
    tokens_with_zero_occurrences = []
    for img in data:
    ...
    print('token counter: {}'.format(token_counter))
...
def merge_duplicate_boxes(data):
    ...
    def arxivdocs_overlap(obj1, obj2):
        b1 = to_x1y1x2y2(obj1)
        b2 = to_x1y1x2y2(obj2)
        iou = IoU(b1, b2)
        if all(b1 == b2) or iou > 0.95: # consider as the same box
            return 1
#        elif (inside(b1, b2) or inside(b2, b1)) \
#                and obj1['names'][0] == obj2['names'][0]: # same object inside the other
#            return 2
...
def sentence_preprocess(phrase):
    ...
    phrase = phrase.lower().translate(translator)
    return phrase
    ...
    for i, info in enumerate(obj_data):
        ...
        if opt is None: # use encode from input file
            if 'split' in info:
                ...
                print("WARNING: no split information given, creating only training set")
...
#num_objects and num_predicates = 0 to disable filtering
def create_roidb(imdb, object_input, relationship_input, metadata_input, object_alias, pred_alias, object_list, pred_list, json_file, h5_file, train_frac, val_frac, num_objects=0, num_predicates=0, min_box_area_frac=0.0001, load_frac=1, use_input_split=True, shuffle=False):
    print('start')
    #pprint.pprint(args)

    obj_alias_dict = {}
    if len(object_alias) > 0:
        print('using object alias from %s' % (object_alias))
        obj_alias_dict, obj_vocab_list = make_alias_dict(object_alias)

    pred_alias_dict = {}
    if len(pred_alias) > 0:
        print('using predicate alias from %s' % (pred_alias))
        pred_alias_dict, pred_vocab_list = make_alias_dict(pred_alias)

    obj_list = []
    if len(object_list) > 0:
        print('using object list from %s' % (object_list))
        obj_list = make_list(object_list)
        #use the same preprocessing that is applied to object and predicate names
        obj_list = [sentence_preprocess(x) for x in obj_list]
        assert(len(obj_list) >= num_objects)

    pred_list = []
    if len(pred_list) > 0:
        print('using predicate list from %s' % (pred_list))
        pred_list = make_list(pred_list)
        #use the same preprocessing that is applied to object and predicate names
        pred_list = [sentence_preprocess(x) for x in pred_list]
        assert(len(obj_list) >= num_predicates)
    ...
    print("rel data: {}, obj data: {}, img data: {}".format(len(rel_data), len(obj_data), len(img_data)))
    assert(len(rel_data) == len(obj_data) and
           len(obj_data) == len(img_data))

    print('read image db from %s' % imdb)
    imdb = h5.File(imdb, 'r')
    num_im, _, _, _ = imdb['images'].shape
    print(f'imdb shape: {num_im}')
    ...
    #TODO: why are there so many duplicate objects in rel data?
    ...
    if min_box_area_frac > 0:
        ...
        print('threshold bounding box by %f area fraction' % min_box_area_frac)
        filter_object_boxes(obj_data, heights, widths, min_box_area_frac) # filter by box dimensions

    #NOTE: merging of duplicate boxes is disabled for testing arxivdocs
    #merge_duplicate_boxes(obj_data)

    # build vocabulary
    object_tokens, object_token_counter = extract_object_token(obj_data, num_objects,
                                                               obj_list, use_zero_freq_tokens=True)

    print('building label to idx from object tokens: {}'.format(object_tokens))
    ...
    # write the h5 file
    print(f'creating h5 file at: {h5_file}')
    f = h5.File(h5_file, 'w')
   ...
   # build train/val/test splits
    ...
    opt = None
    if use_input_split is True:
        opt = {}
        opt['val_begin_idx'] = int(len(obj_data) * train_frac)
        opt['test_begin_idx'] = int(len(obj_data) * val_frac)
        opt['shuffle'] = shuffle
    ...
    print(f'creating json file at: {json_file}')
